...
using ::testing::DoubleNear;
  ...
  // reference file printed to 6 figures
  constexpr double kAbsTol = 1e-5;
  // Test diagonalize
  std::vector<double> eigval(3); PLMD::Matrix<double> eigvec(3,3); 
  diagMat( mat1, eigval, eigvec ); 
  // Instead of dumping to stdout, use matchers:
  //out<<"Eigenvalues "<<eigval[0]<<" "<<eigval[1]<<" "<<eigval[2]<<"\n";
  EXPECT_THAT(eigval, ElementsAre(
        DoubleNear(-0.309693, kAbsTol),
        DoubleNear(0.593857, kAbsTol),
        DoubleNear(1.31584, kAbsTol)));
